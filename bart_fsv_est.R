...
for(i in 1:(T1-T0-h)){
  ...
  model <- stochtree::fsv_mbart(data = y_train, Y_test = y_test, n_ahead = h, lags = 13,bart_prior = "CGM", SV= TRUE, num_burnin = num_burnin, num_mcmc = num_mcmc)
  ...
  saveRDS(saved_info, file = paste0("Results/bartfsv_window_",i,".rds"))
  rm(model)
  gc()
}
