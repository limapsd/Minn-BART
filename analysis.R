...
file_names <- list.files("Results/bckup")
...
LPL_array  <- array(NA, dim = c(estimation_window, h, length(models)),
                        dimnames = list(paste0("w_",1:estimation_window),
                                   paste0("t+",1:h),
                                   models))
LPL_uni_array <- array(NA, dim = c(estimation_window, h, M, length(models)),
                       dimnames = list(paste0("w_", 1:estimation_window),
                                              paste0("t+",1:h),
                                              var.sets$`VAR-22`
                                              ,models))
...
for(m in models){
  ...
  for(w in cmin:cmax){
    if(m == "smallbvar"){
      ...
      saved_model <- readRDS(paste0("Results/bckup/", selected_files))
      ...
      saved_model <- readRDS(paste0("Results/bckup/", selected_files))
      predictions <- saved_model$predictions
      predictions <- predictions[seq(5,nrow(predictions) , by = 5) , ,]
      ...
      LPL_array[w,,m] <- saved_model$LPL
      LPL_uni_array[w,,,m] <- saved_model$LPL_uni
    }
  }
}
...
relative_rmse <- function(rmse_array,rmse_bvar = NULL, model = "bartfsv", variable, round_lvl = 4){
...
plot_model_performance <- function(data,variable_name = NULL) {
  ...
  base_models <- unique(sub("fsv$", "", colnames(data)))  
  colors <- c("black", "blue", "red")[1:length(base_models)]
...
plot_model_performance(rmse_cpi_bvar, variable_name = "INDPROD")
...
LPL_uni<-apply(LPL_uni_array,2:4, sum)
